...
function hasWheel(sortedHand) {
  const wheel = uniqBy(sortedHand.filter(card =>
    card.value === 14 || card.value === 2 || card.value === 3 || card.value === 4 || card.value ===5), 'value');
...
function getSequential(sortedHand) {
  ...
  return uniqSortedHand.filter((card, index, arr) =>
    ...
    && arr[index - 1] && card.value - 1 === arr[index - 1].value)
    || (isSequential(uniqSortedHand.slice(index, index + 5)) && uniqSortedHand.slice(index, index + 5).length === 5)
    || (isSequential(uniqSortedHand.slice(index - 4, index + 1)) && uniqSortedHand.slice(index - 4, index + 1).length === 5)
  );
}
